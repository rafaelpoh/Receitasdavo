...
*   🖼️ **Otimização no Cliente via Canvas:** Antes de carregar qualquer foto para a nuvem, o aplicativo a redimensiona localmente via HTML5 Canvas (mantendo o aspecto original e um limite de 800px). Isso resulta em uploads super-rápidos e economia de armazenamento (fotos de 5MB reduzem para ~80KB sem perda visível de qualidade).
*   ☁️ **Banco de Dados no Cloud Firestore:** Persistência ágil e escalável de todas as suas receitas no Firebase.
*   📁 **Armazenamento no Firebase Storage:** Hospedagem otimizada de imagens, gerando URLs seguras e públicas.
...
*   **Firebase Admin SDK:** Integração direta e segura com o Firestore Database e Firebase Storage.
...
*   Uma conta no [Firebase](https://console.firebase.google.com/) com um projeto configurado (Firestore + Storage ativados).
...
3.  **Configurar variáveis de ambiente:**
    Crie um arquivo `.env` na raiz do projeto contendo as chaves do Firebase e do Gemini:
    ```env
